...
NEV_MODELS = {
    ...
    # —— 2026-09-19 补：8 月期次起这四款已进入出榜名次之内，逐款核实后收录 ——
    '长城H10',         # 2026-07-16 上市，全系第二代 Hi4 插电混动，42.8kWh / CLTC 纯电 180–232km。
                       #   多源一致写明「插电混动」：央广网（2026-07-24）、中国经济网汽车频道、
                       #   新京报贝壳财经、国际金融报。名中无动力后缀，规则3 接不住，故显式收录。
    '星光L',           # 五菱星光L，2026-07-16 上市，六座插混 SUV，1.5L + 170kW 电机，
                       #   37.9kWh / CLTC 纯电 260km。依据：易车参配「能源类型：插电混合」、
                       #   太平洋汽车「能源类型：插电混动」、网通社上市稿。
    '缤果S',           # 五菱缤果S，A0 级五门五座纯电两厢车，CLTC 325/430/525km。
                       #   依据：易车参配「能源类型：纯电」、太平洋汽车「能源类型：纯电动」。
    '领克07GT',        # 2026-07-23 上市，EM-P 智能电混（插混），28.3kWh / CLTC 纯电 200km+，
                       #   综合续航 1422km。依据：央广网（2026-07-24）、河南日报、南方都市报、
                       #   中国经济新闻网，均写明 EM-P 智能电混系统。
}
#
# 以下车型未收录（无可靠依据，退回 UNK 由人工确认后再补）：
#   别克至境E7、星光560、悦也Plus、荣威D6、奥迪E7X、
#   与众06、奔腾小马、悦意03、悦意08
#   它们目前都排在出榜名次之后，不影响 TOP50，只影响 brandTotals 与 UNK 计数。
#
# 2026-09-19 记录：原「未收录」名单里的 缤果S、星光L 已随 8 月期次上量进入出榜名次之内，
#   同时新增 长城H10、领克07GT —— 这四款均已逐款核实动力类型后正式收录（见上）。
#   ⚠️ 这说明「未收录」名单会随时间失效：某款车这个月排在榜外、下个月就可能冲进 TOP50。
#   若抓取再次因 UNK 拒绝写入，按报错里的名次逐一核实即可，不要凭印象放行。
...
#
# 2026-09-19 按 2026年8月 期次重核（官方厂商级数字取站内 static/ev-sales.json 的
# nevMaker 快照，该快照 2026-09-14 已核对，来源为转载乘联分会《2026年8月份全国乘用车
# 市场分析》终稿）：
#   特斯拉 零售 29,260 + 20,787 = 50,047  == 官方零售 50,047  ✓ 逐位闭合
#   特斯拉 批发 55,697 + 30,469 = 86,166  == 官方批发 86,166  ✓ 逐位闭合
#   零跑   零售 本站加总 84,871           vs 官方零售 84,874   ✗ 差 3 辆，未闭合
#   零跑   批发 本站加总 99,330           vs 官方批发 103,129  ✗ 差 3,799 辆，未闭合
#   → 零跑 8 月在售车型已多到有车型跌出抓取深度（300 名之外），
#     不满足「在售车型全部进入榜单」这一锚点前提，故本期不再作为锚点。
#     差额来源未能逐款定位，绝不倒推 expectedDelta 把差额抹平 —— 那是自欺。
#     待抓到车型级完整清单、能逐款点名后再恢复零跑锚点。
# =============================================================================
ANCHORS = [
    {'brand': '特斯拉', 'caliber': 'retail', 'official': 50047,
     'complete': True, 'note': 'Model Y 29,260 + Model 3 20,787，2 款全部在榜'},
    {'brand': '特斯拉', 'caliber': 'wholesale', 'official': 86166,
     'complete': True, 'note': 'Model Y 55,697 + Model 3 30,469，2 款全部在榜'},
]
ANCHOR_PERIOD = '2026年8月'
ANCHOR_VERIFIED_ON = '2026-09-19'
